...
function delete_regdonor(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM regdonors where id = ".$id);
		if($delete){
			return 1;
		}
	}
	...
	function save_donation(){
		...
		$data .= ", date = '$date' ";
			if(empty($id)){
				$save = $this->db->query("INSERT INTO announce set $data");
			}else{
				$save = $this->db->query("UPDATE announce set $data where id = $id");
			}
		if($save)
			return 1;
	}
...
function delete_view_new_comment(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM comment where id = ".$id);
		if($delete){
			return 1;
	...
	}function delete_announcementn(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM announce where id = ".$id);
		if($delete){
			return 1;
		}
}
